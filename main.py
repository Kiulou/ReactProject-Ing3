...
def add_movie_actor(mid: int, sid: int, db: Session = Depends(get_db)):
    ...
    """
    db_actor = crud.add_movie_actor(db=db, movie_id=mid, star_id=sid)
    return None
...
@app.get("/stars/stats2_movie_by_actor")
def read_stats2_movie_by_actor(minc: Optional[int] = 10, db: Session = Depends(get_db)):
    return crud.get_stats2_movie_by_actor(db=db, min_count=minc)
